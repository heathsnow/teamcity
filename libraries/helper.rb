    ...
    def get_file(uri, destination)
      ...
      if File.exists? destination
        if @new_resource.overwrite
          ...
          raise Teamcity::Exceptions::FileAlreadyExists
          "Cannot download file, #{destination} already exists, use :overwrite"
        end
      end
      ...
      Net::HTTP.start(uri.host, uri.port) do |http|
        ...
        http.request req do |response|
          ...
          if response.is_a?(Net::HTTPSuccess)
            ...
            raise Teamcity::Exceptions::FileNotFound
            'The file specified for download does not exist on the server'
          end
        end
      end
    end
