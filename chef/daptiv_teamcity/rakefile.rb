...
task all: %i[version rubocop foodcritic spec kitchen cleanup]
task default: %i[version rubocop foodcritic spec]

desc 'Set cookbook version.'
task :version do
  version = ENV['BUILD_NUMBER'] ? "0.1.#{ENV['BUILD_NUMBER']}" : '0.0.1'
...
desc 'ChefSpec test.'
task :spec do
  load_gems('colorize docker-api minitest-chef-handler')
...
def load_gems(gems)
  gems.split(' ').each do |g|
    begin
      ...
      system "chef gem install docker-api --version '= 1.34.0'"
